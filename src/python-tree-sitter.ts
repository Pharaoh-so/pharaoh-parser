...
function isPublicName(name: string, allNames: Set<string> | null): boolean {
	if (allNames !== null) {
...
function buildSignature(node: SyntaxNode, isAsync: boolean): string {
	const nameNode = node.childForFieldName("name");
...
function countParams(node: SyntaxNode): number {
...
function computeComplexity(node: SyntaxNode): number {
	return computeComplexityShared(node, COMPLEXITY_TYPES, (n) => {
		if (n.type !== "boolean_operator") return false;
