...
function extractFunction(node, source, functions, exports, className) {
    ...
    for (const declarator of node.children) {
        ...
        if (valueNode.type === "arrow_function") {
            ...
            exports.push({
                ...
                kind: "variable",
                isDefault: false,
                className: valueNode.type === "new_expression"
...
function extractExportStatement(node, source, functions, classes, imports, exports) {
    ...
    if (isDefault && !exportClause) {
        const hasDecl = node.children.some((c) => c.type === "function_declaration" ||
            c.type === "class_declaration" ||
            c.type === "lexical_declaration");
        if (!hasDecl) {
            // Default export of an expression (e.g. export default app)
            const exprNode = node.children.find((c) => c.type !== "export" && c.type !== "default" && c.type !== ";");
            exports.push({
...
function extractImport(node, imports) {
 ...
 */
function extractDynamicImports(node, imports) {
    for (const declarator of node.children) {
        if (declarator.type !== "variable_declarator")
            continue;
        const nameNode = declarator.childForFieldName("name");
...
function buildSignature(node, source) {
    ...
    sig += paramsNode?.text ?? "()";
    if (returnTypeNode)
        sig += `: ${returnTypeNode.text.replace(/^:\s*/, "")}`;
    return sig;
}
